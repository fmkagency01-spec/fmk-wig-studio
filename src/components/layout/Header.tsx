      ...
      <div className="border-b">
        <div className="mx-auto max-w-7xl px-4 h-16 flex items-center justify-between gap-4">
          ...
          <nav className="hidden md:flex items-center gap-6 text-sm font-medium">
            ...
            {categories?.slice(0, 3).map((c) => (
              <Link key={c.id} to="/shop" search={{ category: c.slug }} className="hover:text-brand transition-colors">
                {c.name}
              </Link>
            ))}
          </nav>
          ...
          <div className="flex items-center gap-1">
            ...
            <Link to={user ? "/account" : "/auth"} aria-label="Account" className="p-2 hover:text-brand">
        ...
        {open && (
          <nav className="md:hidden border-t bg-background px-4 py-3 flex flex-col gap-2 text-sm">
            ...
            {categories?.map((c) => (
              <Link key={c.id} to="/shop" search={{ category: c.slug }} className="py-2 pl-3 text-muted-foreground">
                {c.name}
              </Link>
            ))}
            {user ? (
              <>
                <Link to="/account" className="py-2">My Account</Link>
                <Link to="/orders" className="py-2">My Orders</Link>
              </>
            ) : (
              <Link to="/auth" className="py-2">
                <Button size="sm" variant="default" className="w-full">Sign in</Button>
              </Link>
            )}
          </nav>
        )}
